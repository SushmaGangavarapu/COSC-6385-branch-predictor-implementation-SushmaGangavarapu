...
public:
	...
	unsigned ip_hash_mask_a;
	unsigned ip_hash_mask_b;
	unsigned ip_hash_mask_c;
	unsigned pir_hash_mask_a;
	unsigned pir_hash_mask_b;
	unsigned taken_mask;
	...
	pm_predictor(void) { // CONSTRUCTOR
		...
		// Create Hash masks
		ip_hash_mask_a = createMask(13, 18);
		ip_hash_mask_b = createMask(4, 12);
		ip_hash_mask_c = createMask(10, 18);
		pir_hash_mask_a = createMask(0, 5);
		pir_hash_mask_b = createMask(6, 14);
		taken_mask = createMask(0, 5);
	} // END OF CONSTRUCTOR
	...
	branch_update* predict(branch_info& b) {
		...
		if (b.br_flags & BR_CONDITIONAL) {
			unsigned  hash = ((((b.address & ip_hash_mask_a) >> 13) ^ (history & pir_hash_mask_a)) << 9) + (((b.address & ip_hash_mask_b) >> 4) ^ ((history & pir_hash_mask_b) >> 6));
	...
	void update(branch_update* u, bool taken, unsigned int target) {
		...
		history = ((history & pir_update_mask) << 2) ^ ((BR_CONDITIONAL * (bi.address & ip_pir_update_mask)) | (BR_INDIRECT * ((bi.address & ip_hash_mask_c >> 5) + (target & taken_mask))));
